...
import { X, Calendar, TrendingUp, ArrowDownCircle, FileText } from 'lucide-react'
import { format } from 'date-fns'
import clsx from 'clsx'

const DEFAULT_FORM = { weekStart: format(new Date(), 'yyyy-MM-dd'), deposit: '', profit: '', note: '' }
...
export default function EntryModal({ open, onClose, onSubmit, initial = null }) {
  ...
  useEffect(() => {
    if (open) {
      setForm(initial ? {
        ...
        deposit: initial.deposit ?? '', profit: initial.profit ?? '', note: initial.note ?? '',
      } : DEFAULT_FORM)
      setError('')
    }
  }, [open, initial])
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try { await onSubmit({ weekStart: form.weekStart, deposit, profit, note: form.note }); onClose() }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
      ...
      <div className="relative bg-surface-card border border-surface-border rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl animate-slide-up">
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-surface-border">
          <h2 className="font-semibold text-zinc-900 dark:text-white">{initial ? "Modifier l'entrée" : 'Nouvelle entrée'}</h2>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors p-1 rounded-lg hover:bg-surface-muted"><X size={18} /></button>
        </div>
        <form onSubmit={handleSubmit} className="px-4 sm:px-6 py-4 sm:py-5 space-y-4 pb-safe">
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-zinc-500 font-medium mb-1.5">Profit (€)</label>
              <div className="relative">
                <TrendingUp size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-500" />
                <input type="number" step="0.01" placeholder="0.00" value={form.profit} onChange={e => set('profit', e.target.value)}
                  className={clsx(inputClass, 'pl-9 pr-3 font-mono')} required />
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 bg-surface-muted hover:opacity-80 border border-surface-border rounded-xl py-2.5 text-sm font-medium transition-all text-zinc-700 dark:text-zinc-300">Annuler</button>
            <button type="submit" disabled={loading} className="flex-1 bg-brand-500 hover:bg-brand-400 text-white font-semibold rounded-xl py-2.5 text-sm transition-all shadow-lg shadow-brand-500/25 disabled:opacity-50">
