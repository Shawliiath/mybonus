  ...
  return (
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-40 -right-40 w-96 h-96 bg-brand-600/8 rounded-full blur-3xl" />
      </div>
      <div className="w-full max-w-md relative animate-slide-up">
        ...
        <div className="bg-surface-card border border-surface-border rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleEmail} className="space-y-4">
            ...
            <Field icon={<Lock size={16} />} type="password" placeholder="Mot de passe" value={password} onChange={e => setPassword(e.target.value)} required />
          ...
          <p className="text-center text-sm text-zinc-500 mt-6">
            ...
            <Link to="/register" className="text-brand-400 hover:text-brand-300 font-medium transition-colors">S'inscrire</Link>
          </p>
        </div>
      </div>
    </div>
...
function getErrorMessage(code) {
  const m = { 'auth/user-not-found': 'Aucun compte avec cet email.', 'auth/wrong-password': 'Mot de passe incorrect.', 'auth/invalid-credential': 'Email ou mot de passe incorrect.', 'auth/too-many-requests': 'Trop de tentatives.', 'auth/popup-closed-by-user': 'Connexion annulée.' }
