...
function Heatmap({ entries, year }) {
  ...
  // Grouper par mois pour les labels
  ...
  return (
    <div>
      {/* Labels mois */}
      <div className="flex gap-1 mb-1 ml-0 text-[10px] text-zinc-600">
        {weeks.map((w, i) => {
          const m = getMonth(w)
        ...
        {weeks.map((w, i) => {
          ...
          return (
            <div
              ...
              className={clsx('w-3 h-3 rounded-sm transition-all cursor-default', getColor(val))}
            />
          )
        })}
      </div>
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-surface-card border border-surface-border rounded-xl px-3 py-2 text-xs shadow-xl">
        ...
        <p className="font-mono font-semibold text-white">{payload[0]?.value?.toLocaleString('fr-FR', { minimumFractionDigits: 2 })} {currency}</p>
      </div>
    )
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <AreaChart data={data} margin={{ top: 5, right: 5, left: -10, bottom: 0 }}>
        ...
        <XAxis dataKey="label" tick={{ fill: '#52525b', fontSize: 11 }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
...
function parseCSVImport(text) {
  ...
  lines.forEach((line, i) => {
    ...
    if (!weekStart || isNaN(deposit) || isNaN(profit)) {
      errors.push(`Ligne ${i + 1}: données invalides (${line.slice(0, 40)})`)
      return
    }
    // Valider format date
    if (!/^\d{4}-\d{2}-\d{2}/.test(weekStart)) {
      errors.push(`Ligne ${i + 1}: date invalide — utiliser le format YYYY-MM-DD`)
      return
    }
    ...
    <div>
      {status === 'idle' && (
        <div>
          <p className="text-xs text-zinc-500 mb-3">
            Format CSV attendu : <code className="bg-surface-muted px-1.5 py-0.5 rounded text-zinc-300">YYYY-MM-DD;dépôt;profit;note</code>
      ...
      {status === 'preview' && (
        <div className="space-y-3">
          ...
          {preview.length > 0 && (
            <div>
              <p className="text-xs text-zinc-500 mb-2">{preview.length} ligne{preview.length > 1 ? 's' : ''} prête{preview.length > 1 ? 's' : ''} à importer :</p>
              <div className="max-h-48 overflow-y-auto space-y-1">
                {preview.slice(0, 10).map((r, i) => (
                  <div key={i} className="flex items-center justify-between text-xs bg-surface-muted rounded-lg px-3 py-2">
                    <span className="text-zinc-400">{r.weekStart}</span>
                    <span className="text-zinc-500">{r.deposit} dép.</span>
                    <span className={r.profit >= 0 ? 'text-brand-400' : 'text-red-400'}>
                      {r.profit >= 0 ? '+' : ''}{r.profit}
                    </span>
                  </div>
                ))}
          ...
          {preview.length === 0 && errors.length > 0 && (
            <button onClick={() => { setStatus('idle'); setErrors([]) }}
              className="text-xs text-zinc-500 underline">Réessayer</button>
          )}
        </div>
      )}
      ...
      {status === 'done' && (
        <p className="text-sm text-brand-400">✓ Import terminé !</p>
      )}
    </div>
  )
}
...
export default function Analytics() {
  ...
  const yearEntries = useMemo(() => entries.filter(e => {
    if (!e.weekStart) return false
    return getYear(new Date(e.weekStart)) === year
  }), [entries, year])
      ...
      <div className="px-6 py-8 max-w-4xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-0.5">Vue approfondie de tes performances</p>
          </div>
          <select value={year} onChange={e => setYear(Number(e.target.value))}
            className="bg-surface-muted border border-surface-border rounded-xl px-3 py-1.5 text-sm text-white focus:outline-none transition-all">
        ...
        {/* Streak */}
        {streak > 0 && (
          <div className="bg-surface-card border border-amber-500/20 rounded-2xl px-5 py-4 flex items-center gap-4">
            <div className="w-10 h-10 bg-amber-500/10 rounded-xl flex items-center justify-center">
              <Flame size={18} className="text-amber-400" />
            </div>
            <div>
              <p className="text-lg font-bold text-white font-mono">{streak} semaine{streak > 1 ? 's' : ''} 🔥</p>
        ...
        <div className="bg-surface-card border border-surface-border rounded-2xl p-5">
          <h2 className="text-sm font-semibold text-zinc-400 mb-5 flex items-center gap-2">
        ...
        {/* Heatmap */}
        <div className="bg-surface-card border border-surface-border rounded-2xl p-5">
          <h2 className="text-sm font-semibold text-zinc-400 mb-5">Heatmap {year}</h2>
          {loading
